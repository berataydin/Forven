...
# AI-01 (audit 2026-07-25): the ``run_code`` tool description promised "No
# network access" while the child was spawned with a plain interpreter and full
# egress — a lie an agent (or a prompt injection reaching the tool) could act on
# to exfiltrate whatever it could read. This bootstrap installs the SAME
# outbound-socket deny the strategy worker uses (see
# ``strategy_worker._install_network_deny``) and then executes the submitted
# script as ``__main__``.
#
# It is duplicated rather than imported because the POSIX branch below hands the
# child an env with no PYTHONPATH — it cannot ``import forven`` at all — so the
# bootstrap has to be self-contained. The worker's copy stays the canonical one;
# if that primitive is ever promoted to a shared module, collapse this into it.
#
# The wrapper also re-raises the user's traceback with the bootstrap/runpy frames
# stripped, so an agent still sees the line numbers of the code IT submitted.
_SANDBOX_RUNNER_SOURCE = '''\
"""Sandbox bootstrap — generated by forven.sandbox.run_code. Do not edit."""
import runpy
import sys
import traceback


def _install_network_deny():
    """Make ALL outbound socket connects raise inside the sandbox child.

    Loopback is denied too: sandboxed code needs no network of any kind, and a
    guard bypass must not reach the local control plane (127.0.0.1:8003) for
    SSRF against the trusted host."""
    try:
        import socket
    except Exception:
        return
    ...
    def _deny(*_a, **_k):
        raise OSError("network access is disabled in the code sandbox")
...
try:
    ...
    _clean = _tb
    while _clean is not None and _clean.tb_frame.f_code.co_filename != _target:
        _clean = _clean.tb_next
    traceback.print_exception(_exc_type, _exc, _clean if _clean is not None else _tb,
                              file=sys.stderr)
    sys.exit(1)
'''

# Basename of the submitted script inside the throwaway working directory. Kept
# distinctive so it can never shadow a stdlib/third-party module on sys.path[0].
_SANDBOX_SCRIPT_NAME = "forven_sandbox_main.py"
_SANDBOX_RUNNER_NAME = "_forven_sandbox_runner.py"


def _prepare_sandbox_workdir(code: str) -> tuple[tempfile.TemporaryDirectory, str, str]:
    """Write *code* + the network-deny bootstrap into a throwaway directory.

    Returns ``(tmpdir, runner_path, script_path)``. The caller owns ``tmpdir``
    and must call ``.cleanup()``.
    """
    tmpdir = tempfile.TemporaryDirectory(prefix="forven_sandbox_", ignore_cleanup_errors=True)
    script_path = str(Path(tmpdir.name) / _SANDBOX_SCRIPT_NAME)
    runner_path = str(Path(tmpdir.name) / _SANDBOX_RUNNER_NAME)
    Path(script_path).write_text(code, encoding="utf-8", newline="\n")
    Path(runner_path).write_text(_SANDBOX_RUNNER_SOURCE, encoding="utf-8", newline="\n")
    return tmpdir, runner_path, script_path
...
def run_code(
    ...
    """
    # AI-01: the child runs in a THROWAWAY working directory, never the repo
    # root. cwd=REPO_ROOT meant any relative path the submitted code opened
    # (``open("forven/secret_storage.py")``, ``open(".env")``, a write into
    # ``custom/``) resolved straight into the live checkout. Combined with the
    # network deny installed by the bootstrap below, the tool description's
    # sandbox claims are now true rather than aspirational.
    tmpdir, runner_path, script_path = _prepare_sandbox_workdir(code)
    workdir = tmpdir.name
    ...
    if IS_WINDOWS:
        cmd = [PYTHON_EXE, runner_path, script_path]
