"""Robustness engine — the domain logic behind the validation suite.

Walk-forward, Monte Carlo, parameter-jitter, cost-stress and regime-split
analysis; artifact validation; the composite robustness score; verdict
reconciliation; and the inline/submit result runners that persist every
artifact the paper gate reads.

ARCH-03: all of this used to live inside ``forven.routers.robustness``. The
consequence was that the autonomous gauntlet — the thing that decides whether a
strategy reaches paper — could only invoke its own validation suite by importing
the web layer and calling a decorated FastAPI endpoint. The endpoints now sit on
top of this module and add nothing but routing, so an HTTP call and a gauntlet
call run byte-identical code.

IMPORT-TIME SIDE EFFECTS ARE FORBIDDEN HERE (JOB-SWEEP-1). This module is
imported by every spawn-context pool worker that unpickles
``_monte_carlo_bootstrap_worker`` / ``_regime_classify_trades_worker``, so
anything that runs at import runs once per pool spawn against the LIVE database.
``_cleanup_orphaned_running_jobs`` is called from the API startup hook only.
"""
...
# The logger CHANNEL is deliberately unchanged by the ARCH-03 split.
# ``_log_robustness_finalized`` emits a shape-stable single-line record that
# downstream grep/SIEM rules pin to, and those rules match the channel as well as
# the message — renaming it would silently break them with no test failure.
...
# ---------------------------------------------------------------------------
# Public entry points — the ONLY thing callers outside this module should use.
#
# ARCH-03: before the split, running the validation suite meant importing
# forven.routers.robustness and calling a decorated FastAPI endpoint, so the
# gauntlet (gauntlet/tasks.py) and the evolution loop both depended on the web
# layer. The HTTP endpoints now delegate straight to these and add nothing, so
# an HTTP request and an autonomous gauntlet step run the same code path.
#
# Each accepts either the typed body from forven.robustness.models or plain
# keyword arguments, so a caller that would rather not touch pydantic doesn't
# have to.
#
# ``*_inline`` runs the analysis on the calling thread and returns the result.
# ``*_submit`` hands it to the background executor and returns a job handle.
# ---------------------------------------------------------------------------


def run_walk_forward_inline(body: WalkForwardBody | None = None, **kwargs) -> dict:
    """Run walk-forward analysis now; persist the artifact; return the result."""
    body = body if body is not None else WalkForwardBody(**kwargs)
...
def run_walk_forward_submit(body: WalkForwardBody | None = None, **kwargs) -> dict:
    """Queue walk-forward analysis on the background executor."""
    body = body if body is not None else WalkForwardBody(**kwargs)
...
def run_monte_carlo_inline(body: MonteCarloBody | None = None, **kwargs) -> dict:
    """Run the Monte Carlo bootstrap now; persist the artifact; return the result."""
    body = body if body is not None else MonteCarloBody(**kwargs)
    return _run_inline_result(
...
def run_monte_carlo_submit(body: MonteCarloBody | None = None, **kwargs) -> dict:
    """Queue the Monte Carlo bootstrap on the background executor."""
    body = body if body is not None else MonteCarloBody(**kwargs)
    return _submit_result(
        result_type="monte_carlo",
        context=_prepare_monte_carlo_context(body),
...
def run_param_jitter_inline(body: ParamJitterBody | None = None, **kwargs) -> dict:
    """Run the parameter-jitter sweep now; persist the artifact; return the result."""
    body = body if body is not None else ParamJitterBody(**kwargs)
    return _run_inline_result(
...
def run_param_jitter_submit(body: ParamJitterBody | None = None, **kwargs) -> dict:
    """Queue the parameter-jitter sweep on the background executor."""
    body = body if body is not None else ParamJitterBody(**kwargs)
    return _submit_result(
        result_type="param_jitter",
        context=_prepare_param_jitter_context(body),
...
def run_cost_stress_inline(body: CostStressBody | None = None, **kwargs) -> dict:
    """Run the cost-stress rerun now; persist the artifact; return the result."""
    body = body if body is not None else CostStressBody(**kwargs)
    return _run_inline_result(
...
def run_cost_stress_submit(body: CostStressBody | None = None, **kwargs) -> dict:
    """Queue the cost-stress rerun on the background executor."""
    body = body if body is not None else CostStressBody(**kwargs)
    return _submit_result(
        result_type="cost_stress",
        context=_prepare_cost_stress_context(body),
...
def run_regime_split_inline(body: RegimeSplitBody | None = None, **kwargs) -> dict:
    """Run the regime split now; persist the artifact; return the result."""
    body = body if body is not None else RegimeSplitBody(**kwargs)
    return _run_inline_result(
...
def run_regime_split_submit(body: RegimeSplitBody | None = None, **kwargs) -> dict:
    """Queue the regime split on the background executor."""
    body = body if body is not None else RegimeSplitBody(**kwargs)
    return _submit_result(
        result_type="regime_split",
        context=_prepare_regime_split_context(body),
