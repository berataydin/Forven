...
import { fmtAgeSeconds, fmtNum, fmtUsd } from '../lib/components/dashboard_preview/format';
...
import DashboardPreviewPage from '../routes/dashboard-preview/+page.svelte';
...
describe('dashboard preview page', () => {
	it('renders tiers from one snapshot with truthful chips and null-safe KPIs', async () => {
		...
		mountComponent(DashboardPreviewPage, {});
		await flush();
		...
		expect(target.querySelector('[data-testid="preview-data-chip-stale"]')).not.toBeNull();
		expect(target.querySelector('[data-testid="preview-equity-chip-error"]')).not.toBeNull();
